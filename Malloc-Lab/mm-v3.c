...
int mm_init(void) {
    ...
    PUT_WORD(heap_listp + (9  * WSIZE), PACK_SIZE_ALLOC(DSIZE, 1));
    PUT_WORD(heap_listp + (10 * WSIZE), PACK_SIZE_ALLOC(DSIZE, 1));
    PUT_WORD(heap_listp + (11 * WSIZE), PACK_SIZE_ALLOC(0, 1));
...
static void* extend_heap(size_t words) {
    ...
    PUT_WORD(PRED_PTR(bp), 0);
    PUT_WORD(SUCC_PTR(bp), 0);
...
void* get_head_ptr(size_t size){
    ...
    char *head_ptr;
    if(cnt <= 5){
        head_ptr = (char *)(heap_listp) - 2 * WSIZE;
    }
    else if(cnt >= 13){
        head_ptr = (char *)(heap_listp) - 10 * WSIZE;
    }
    else{
        head_ptr = (char *)(heap_listp) - (cnt - 3) * WSIZE;
    }
    return head_ptr;
}
...
static void* find_fit_by_head(size_t asize, void *head_ptr){
    // 第一个空闲块，往后搜寻
    // 获取第一个空闲块地址
    char *first_free_block_bp = GET_WORD(head_ptr);
...
static void* find_fit(size_t asize) {
    char *end_head = heap_listp - 11 * WSIZE;
    char *head_ptr;
    char *ans;
    for(head_ptr = get_head_ptr(asize);head_ptr != end_head;head_ptr = head_ptr - WSIZE){
        if ((ans = find_fit_by_head(asize, head_ptr)) != NULL){
            return ans;
        }
    }
    return NULL;
...
void insert_free_block_to_head(void *bp, void *head_ptr){
    // 获取第一个空闲块地址
    char *first_free_block_bp = GET_WORD(head_ptr);
    // 如果之前没有空闲块
    if(first_free_block_bp == NULL){
        // 更新第一个空闲块的地址
        PUT_WORD(head_ptr, bp);
        // 第一个空闲块的前驱指0
        PUT_WORD(PRED_PTR(bp), NULL);
        // 第一个空闲块的后继指0
        PUT_WORD(SUCC_PTR(bp), NULL);
...
void insert_free_block(void *bp){
    ...
    int block_size = GET_BLOCK_SIZE(HEADER_PTR(bp));
    char *head_ptr = get_head_ptr(block_size);
    insert_free_block_to_head(bp, head_ptr);
}

void delete_free_block_from_head(void *bp, void *head_ptr){
    char *first_free_block_bp = GET_WORD(head_ptr);
    ...
    // 如果是删除第一个
        ...
        PUT_WORD(PRED_PTR(succ_block_bp), pred_block_bp);
        PUT_WORD(head_ptr, succ_block_bp);
    }
}
...
void delete_free_block(void *bp){
    ...
    int block_size = GET_BLOCK_SIZE(HEADER_PTR(bp));
    char *head_ptr = get_head_ptr(block_size);
    delete_free_block_from_head(bp, head_ptr);
}
